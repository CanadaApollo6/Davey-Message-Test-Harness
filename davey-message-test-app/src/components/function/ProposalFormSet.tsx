...
const ProposalFormSet: React.FC<ProposalFormSetProps> = ({ setMessage }) => {
  ...
  const constructMessageText = () => {
    ...
    switch (proposalType) {
      case ProposalType.Proposal: {
        proposalInfo = `Proposal Number: 123456 <br /> Presented Date: ${new Date().toLocaleDateString()}`;
        break;
      }
      case ProposalType.DefaultRenewal: {
        proposalInfo = `Proposal Number: 123456 <br /> Renewal Year: ${new Date().getFullYear()} <br /> Renewal Type: NEV`;
        break;
      }
      case ProposalType.EVGRenewal: {
        proposalInfo = `Proposal Number: 123456 <br /> Renewal Year: ${new Date().getFullYear()} <br /> Renewal Type: EVG`;
        break;
      }
    }
    ...
    const messageText = `${mainResponse} <br /> ${proposalInfo} <br /> ${declineReasonText} <br /> ${formComments}`;
    return messageText;
  };
  ...
  const getCorrectForm = (proposalType: ProposalType) => {
    switch (proposalType) {
      case ProposalType.Proposal:
        return (
          <StandardProposalFrom proposalFormControls={proposalFormControls} />
        );
      case ProposalType.EVGRenewal:
        return <EVGRenewalForm proposalFormControls={proposalFormControls} />;
      case ProposalType.DefaultRenewal:
        return (
          <DefaultRenewalForm proposalFormControls={proposalFormControls} />
        );
    }
  };
